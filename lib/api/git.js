        ...
        if ( res.data ) {
            for ( const [ tag, commit ] of Object.entries( res.data ) ) {
                ...
                if ( /^v\d+\.\d+\.\d+/.test( tag ) ) {
                    ...
                    const release = new SemanticVersion( tag );

                    // next
                    tags.next ??= {};

                    if ( tags.next.version ) {
                        if ( release.gt( tags.next.version ) ) {
                            tags.next.version = release;
                        }
                    }
                    else {
                        tags.next.version = release;
                    ...
                    if ( !release.isPreRelease ) {
                        ...
                        if ( tags.latest.version ) {
                            if ( release.gt( tags.latest.version ) ) {
                                tags.latest.version = release;
                            }
                        }
                        else {
                            tags.latest.version = release;
                        ...
                        // major
                        const majorTag = "v" + release.majorNumber;

                        tags[ majorTag ] ??= {};

                        if ( tags[ majorTag ].version ) {
                            if ( release.gt( tags[ majorTag ].version ) ) {
                                tags[ majorTag ].version = release;
                            }
                        }
                        else {
                            tags[ majorTag ].version = release;
                ...
                // release tag
                else if ( tag === "latest" || tag === "next" || /^v\d+$/.test( tag ) ) {
                    tags[ tag ] ??= {};
