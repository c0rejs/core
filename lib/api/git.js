...
function compareBranches ( a, b ) {
    ...
    // master
        ...
        const aVersion = a.match( /^(v\d+)\.x$/v )?.[ 1 ],
            bVersion = b.match( /^(v\d+)\.x$/v )?.[ 1 ];
...
export default class Git {
    ...
    async getWorkingTreeStatus () {
        ...
        if ( !res.ok ) {
            ...
            for ( const line of res.data.split( "\0" ) ) {
                ...
                const match = line.match( /^(?<index>.)(?<workingTree>.) (?<file>.+)$/v );
    ...
    async getBranchStatus () {
        ...
        for ( const line of res.data.split( "\n" ) ) {
            ...
            const [ branch, upstream, status ] = line.split( "\0", 3 );

            data[ branch ] = {
                "upstream": upstream || null,
                "synchronized": null,
                "ahead": Number( status.match( /ahead (\d+)/v )?.[ 1 ] || 0 ),
                "behind": Number( status.match( /behind (\d+)/v )?.[ 1 ] || 0 ),
            };
    ...
    async getReleases () {
        ...
        if ( res.ok ) {
            const tags = res.data
                ...
                .filter( line => line )
                .map( line => {
                    const [ version, date ] = line.split( "\0", 2 );
    ...
    async getCommits ( commitsRange, { maxCount, tags } = {} ) {
        ...
        // prepare git command
        const rowsSeparator = Uuid.v4(),
            separator = Uuid.v4(),
            decorateSeparator = Uuid.v4(),
            tagId = Uuid.v4(),
    ...
    async #findPreviousRelease ( { commitRef, stable } ) {
        ...
        const match = res.data.trim().match( /^(?<version>v\d+\.\d+\.\d.*)-(?<distance>\d+)-g(?<abbrev>[\da-f]+)$/v );
