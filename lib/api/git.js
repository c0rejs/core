...
export default class Git {
    ...
    async getStatus ( { branchStatus } = {} ) {
        ...
        counter.value++;
        this.getParentRelease().then( res => {
            if ( res.ok ) {
                status.head = res.data.commit;
                status.releases = res.data.releases;
                status.parentRelease = res.data.parentRelease;
                status.parentReleaseDistance = res.data.parentReleaseDistance;
            }
            else {
                error = res;
            }
    ...
    async getBuildVersion ( { commitRef } = {} ) {
        ...
        res = await this.getParentRelease( { commitRef } );
        if ( !res.ok ) return res;
        const parentRelease = res.data;

        var version = ( parentRelease.parentRelease || SemanticVersion.initialVersion ).versionString;

        version += "+" + parentRelease.commit.abbrev;

        if ( parentRelease.parentReleaseDistance ) {
            version += "." + parentRelease.parentReleaseDistance;
        }
    ...
    async getParentRelease ( { commitRef, stable, changes } = {} ) {
        ...
        if ( commit.isRelease ) {
            if ( stable ) {
                if ( commit.releaseVersion.isStableRelease ) {
                    data.parentRelease = releases.get( commit.releaseVersion );
                }
            }
            else {
                data.parentRelease = releases.get( commit.releaseVersion );
        ...
        if ( !data.parentRelease ) {
            ...
            if ( res.data ) {
                data.parentRelease = releases.get( res.data.version );
                data.parentReleaseDistance = res.data.distance;
            }
            else {
                res = await this.getCommitsDistance( commit.hash );
                if ( !res.ok ) return res;

                data.parentReleaseDistance = res.data;
            }
        }

        // get changes
        if ( changes ) {
            res = await this.getChanges( [ data.parentRelease, commit.hash ] );
            if ( !res.ok ) return res;
