...
class SmtpConnection {
    ...
    async auth () {
        ...
        const sasl = await Sasl.new( this.#extensions.AUTH.split( /\s+/v ), this.#smtp.username, this.#smtp.password );
    ...
    async data ( { to, from, replyTo, cc, bcc, subject, text, html, attachments } ) {
        ...
        await using body = text || html || attachments
            ? new FormData( { "type": "mixed" } )
            : undefined;

        await using content = text || html
            ? new FormData( { "type": "alternative" } )
            : undefined;
        ...
        if ( body ) {
            if ( content ) {
                if ( text ) {
                    content.write( new Message( {
                        "headers": {
                            "content-type": "text/plain; charset=utf-8",
                        },
                        "body": text,
                    } ).addMiddleware( this.#encodeBase64.bind( this ) ) );
                }
        ...
        this.#write( headers.toBuffer( { "crlf": true } ) );
    ...
    async #ehlo ( hostname ) {
        ...
        for ( const line of res.data ) {
            const match = line.match( /^([\dA-Z]+)\s?(.*)/v );
    ...
    async #encodeBase64 ( message ) {
        const body = await message.createBody();

        message.headers.set( "content-transfer-encoding", "base64" );

        message.setBody( Buffer.isBuffer( body )
            ? Buffer.from( body.toString( "base64" ) )
            : stream.pipeline( body, new Base64StreamEncoder(), () => {} ) );
    }
}
