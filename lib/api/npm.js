...
export default class Npm {
    ...
    async getPackageVersions ( packageName, { cwd, registry } = {} ) {
        ...
        if ( res.ok && Array.isArray( res.data ) && Array.isArray( res.data[ 0 ] ) ) {
            res.data = res.data[ 0 ];
        }
    ...
    async updateDependencies ( { cwd, registry, dryRun, ignoreScripts, ignorePackageLock } = {} ) {
        ...
        return this.exec( args, { cwd, registry } );
    }
    ...
    async pack ( { cwd, executablesPatterns } = {} ) {
        ...
        const { name } = await readConfig( ( cwd ?? this.#cwd ) + "/package.json" );
        ...
        const filename = Array.isArray( res.data )
                ? res.data[ 0 ].filename // npm < 12.0.0
