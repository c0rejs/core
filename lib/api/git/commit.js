...
export default class GitCommit extends GitMessage {
    ...
    get fixes () {
        if ( this.#fixes === undefined ) {
            const links = Array.from( this.message.matchAll( /(?:^|\W)(?:close[ds]?|fix(?:es|ed)?|resolve(?:s|ds)?) *:? +((?:[\w.-]+\/[\w.-]+)?#\d+)(?:\W|$)/gim ), match => match[ 1 ] );
    ...
    get links () {
        if ( this.#links === undefined ) {
            const links = Array.from( this.message.matchAll( /(?:^|\W)((?:[\w.-]+\/[\w.-]+)?#\d+)(?:\W|$)/gm ), match => match[ 1 ] );
