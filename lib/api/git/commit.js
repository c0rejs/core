...
const COMMIT_SUBJECT_RE = /^(?<type>[\da-z\-]+)(?:\((?<scope>[\da-z\-]+)\))?(?<breaking>!)?: (?<subjectText>.+)/v;
...
export default class GitCommit extends GitMessage {
    ...
    constructor ( { message, hash, abbrev, date, author, isHead, branch, branches, tags, parentHashes } ) {
        ...
        if ( body ) {
            ...
            // footers found
                ...
                const footers = body.slice( match.index ).split( /^(breaking[ \-]change|[\da-z\-]+) *: */imv );

                for ( let n = 1; n < footers.length; n += 2 ) {

                    // breaking change footer
                    if ( /^breaking[ \-]change$/iv.test( footers[ n ] ) ) {
    ...
    get revertHash () {
        if ( this.#revertHash === undefined ) {
            ...
            if ( this.isRevert ) {
                const match = this.body.match( /This reverts commit ([\da-f]+)/v );
    ...
    get fixes () {
        if ( this.#fixes === undefined ) {
            const links = Array.from( this.message.matchAll( /(?:^|\W)(?:close[ds]?|fix(?:es|ed)?|resolve(?:s|ds)?)(?: +(?:: +)?|: +)((?:[\w.\-]+\/[\w.\-]+)?#\d+)(?:\W|$)/gimv ), match => match[ 1 ] );
    ...
    get links () {
        if ( this.#links === undefined ) {
            const links = Array.from( this.message.matchAll( /(?:^|\W)((?:[\w.\-]+\/[\w.\-]+)?#\d+)(?:\W|$)/gmv ), match => match[ 1 ] );
    ...
    get releaseVersion () {
        if ( this.#releaseVersion === undefined ) {
            ...
            for ( const tag of this.tags ) {
                if ( /^v\d+\.\d+\.\d+/v.test( tag ) && SemanticVersion.isValid( tag ) ) {
