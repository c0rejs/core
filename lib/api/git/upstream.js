...
const ISSUE_RE = /(?<=^|\W)(?<repositorySlug>[\w.\-]+\/[\w.\-]+)?#(?<issueId>\d+)(?=\W|$)/gv,
    COMMIT_RE = /(?<=^|\W)(?:(?<repositorySlug>[\w.\-]+\/[\w.\-]+)@)?(?<commitHash>[\da-f]{7,40})(?=\W|$)/gv;
...
export default class GitUpstream {
    ...
    constructor ( url ) {
        var match = url.match( /^git@([\d.A-Za-z\-]+):([\w\-]+)\/([\w.\-]+)/v );
        ...
        // ssh://git@github.com/softvisio/phonegap.git
            ...
            match = url.pathname.match( /([\w\-]+)\/([\w\-]+)/v );
        ...
        this.#repositoryName = this.repositoryName.replace( /\.git$/v, "" );
