...
export default class StreamSlicer extends stream.Transform {
    ...
    static slice ( message, { range } = {} ) {
        ...
        const bodyStream = message.createBodyStreamSync();
        ...
        if ( range ) {
            ...
            const sliceStream = new this( range ) //
                ...
                .setFilename( message.contentDisposition.filename )
                .setLastModified( message.headers.lastModified.date );

            return stream.pipeline( bodyStream, sliceStream, e => {} );
        }
        else {
            return bodyStream;
        }
