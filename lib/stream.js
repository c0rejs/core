    ...
    "readChunk1": {
        ...
        async value ( chunkLength, { encoding } = {} ) {
            ...
            if ( this.destroyed ) {
                ...
                for await ( let buffer of this.iterator( { "destroyOnReturn": false } ) ) {
                    if ( !Buffer.isBuffer( buffer ) ) buffer = Buffer.from( buffer );
                    ...
                    if ( length >= chunkLength ) {
                        ...
                        if ( buffer.length > chunkLength ) {
                            ...
                            buffer = buffer.subarray( 0, chunkLength );
                        }
    ...
    "readLine1": {
        ...
        async value ( { eol, maxLength, encoding, lastEolRequired = true } = {} ) {
            ...
            if ( this.destroyed ) {
                ...
                for await ( let chunk of this.iterator( { "destroyOnReturn": false } ) ) {
                    if ( !Buffer.isBuffer( chunk ) ) chunk = Buffer.from( chunk );
                    ...
                    if ( idx > -1 ) {
                        ...
                        // max length
                        if ( maxLength && idx > maxLength ) {
                            this.destroy();

                            return;
                        }
                        else {
                            this.unshift( buffer.subarray( idx + eol.length ) );
                ...
                // eol not found
                if ( buffer?.length && !lastEolRequired ) {
                    return encoding
                        ? buffer.toString( encoding )
                        : buffer;
                }
