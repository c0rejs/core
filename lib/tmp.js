...
export class TmpFile extends Tmp( File ) {
    ...
    constructor ( filePath ) {
        ...
        if ( isPlainObject( filePath ) ) {
            ( { "path": filePath, tmpDir, name, extname, type, lastModified } = filePath );
        }
        ...
        filePath ||= path.join( tmpDir || defaultTmpDir, Uuid.v4() + ( extname || "" ) );
    ...
    // static
    static new ( tmpFile ) {
        if ( tmpFile instanceof this ) {
            return tmpFile;
        }
        else {
            return new this( tmpFile );
        }
    }
...
export class TmpDir extends Tmp() {
    constructor ( dirPath ) {
        ...
        dirPath ||= path.join( tmpDir || defaultTmpDir, Uuid.v4() );
