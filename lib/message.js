...
export default class Message {
    ...
    get isReusableBody () {
        return this.hasBody && !this.#isFunctionBody && !this.#isStreamBody;
    }
    ...
    addLastModifiedFromBody () {
        if ( this.#isFileBody && !this.#headers.has( "last-modified" ) ) {
    ...
    async createBody ( { range, createStream } = {} ) {
        ...
        // Buffer
            ...
            if ( createStream ) {
                body = stream.Readable.from( body, { "objectMode": false } ).setContentLength( body.length ).setContentType( this.contentType );
            }
    ...
    async toFileBody () {
        ...
        if ( Buffer.isBuffer( body ) ) {
            ...
            await stream.pipeline( body, fs.createWriteStream( tmpFile.path ) );
        }
    ...
    #createBodySync ( { range, createStream } = {} ) {
        ...
        if ( this.#isFunctionBody ) throw new Error( "Budy is function" );
    ...
    // ranges are supported
        ...
        // multiple ranges
            ...
            for ( const range of ranges ) {
                ...
                if ( message.isFunctionBody || message.isFileBody ) {
