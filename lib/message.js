...
export default class Message {
    ...
    get realContentLength () {
        if ( this.#isFunction || this.#middlewares.length ) {
            return;
        }
        else {
            return this.#headers.contentLength.bytes;
    ...
    setBody ( body, { encoding } = {} ) {
        ...
        // function
            ...
            if ( this.#runningMiddleware ) throw new Error( "Message body function added from transformer" );
    ...
    addMiddleware ( transformerFunction ) {
        if ( this.#isDestroyed ) throw new Error( "Message is destroyed" );

        if ( this.#runningMiddleware ) throw new Error( "Message transformer added from other transformer" );

        if ( transformerFunction ) {
            if ( typeof transformerFunction !== "function" ) throw new Error( "Transformer is not a function" );

            this.#middlewares.push( transformerFunction );
        }
    ...
    async #runMiddlewares () {
        if ( this.#runningMiddleware ) throw new Error( "Message transformer called from other transformer" );

        if ( !this.#middlewares.length ) return;

        while ( true ) {
            const transformerFunction = this.#middlewares.shift();
