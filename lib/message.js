...
export default class Message {
    ...
    addContentEncodingMiddleware ( { headers, compress, zlibOptions } = {} ) {
        ...
        if ( headers ) {
            ...
            for ( encoding of acceptEncodings ) {
                if ( encoding in CONTENT_ENCODINGS_ALGORITHM ) break;
            }
        ...
        if ( !CONTENT_ENCODINGS_ALGORITHM[ encoding ] ) return this;
    ...
    addTransferEncodingMiddleware ( { headers, compress, zlibOptions } = {} ) {
        ...
        if ( headers ) {
            ...
            for ( encoding of te ) {
                if ( encoding in TRANSFER_ENCODINGS_ALGORITHM ) break;
            }
        ...
        if ( !TRANSFER_ENCODINGS_ALGORITHM[ encoding ] ) return this;
...
async function contentEncodingMiddleware ( message, encoding, { compress, zlibOptions } ) {
    ...
    const comressedBody = await zlib.compress( CONTENT_ENCODINGS_ALGORITHM[ encoding ], message, zlibOptions );
...
async function transferEncodingMiddleware ( message, encoding, { compress, zlibOptions } ) {
    ...
    const body = await message.createBodyStream(),
        comressedBody = await zlib.compress( TRANSFER_ENCODINGS_ALGORITHM[ encoding ], body, zlibOptions );
