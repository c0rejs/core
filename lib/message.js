...
async function compressionTransformer ( message, encoding, compressor, { compress, zlibOptions } ) {
...
async function rangeTransformer ( message, ranges, { headers, maxRanges } ) {
    if ( message.isDestroyed ) return;
        ...
        // multiple ranges
            ...
            for ( const range of ranges ) {
                ...
                if ( message.isFunction || message.isFile ) {
                    ...
                    rangeBody = async () => body( { range } );
                }
                else {
                    rangeBody = await message.createBody( { range } );
                }
...
export default class Message {
    ...
    get realContentLength () {
        if ( this.#isFunction || this.#transformers.length ) {
            return;
        }
        else {
            return this.#headers.contentLength.bytes;
    ...
    setBody ( body, { encoding } = {} ) {
        ...
        // function
            ...
            if ( this.#runningTransformer ) throw new Error( "Message body function added from transformer" );
    ...
    addTransformer ( transformerFunction ) {
        if ( this.#isDestroyed ) throw new Error( "Message is destroyed" );

        if ( this.#runningTransformer ) throw new Error( "Message transformer added from other transformer" );

        if ( transformerFunction ) {
            if ( typeof transformerFunction !== "function" ) throw new Error( "Transformer is not a function" );

            this.#transformers.push( transformerFunction );
        }

        return this;
    }

    addCompressionTransformer ( { headers, compress, zlibOptions } = {} ) {
        if ( this.#isDestroyed ) throw new Error( "Message is destroyed" );
        ...
        this.addTransformer( async message =>
            compressionTransformer( message, encoding, compressor, {
                compress,
                zlibOptions,
            } ) );
    ...
    addRangeTransformer ( { ranges, headers, maxRanges } = {} ) {
        if ( this.#isDestroyed ) throw new Error( "Message is destroyed" );
        ...
        this.addTransformer( async message =>
            rangeTransformer( message, ranges, {
                headers,
                maxRanges,
            } ) );
    ...
    async transformMessage () {
        if ( this.#runningTransformer ) throw new Error( "Message transformer called from other transformer" );

        if ( !this.#transformers.length ) return;

        while ( true ) {
            const transformerFunction = this.#transformers.shift();

            if ( !transformerFunction ) break;

            this.#runningTransformer = true;

            // call transformer
            try {
                await transformerFunction( this );

                this.#runningTransformer = false;
            }
            catch ( e ) {

                // error calling transformer
                this.#runningTransformer = false;

                await this.#onTransformError( e );
            }
        }
    }
    ...
    async createBody ( { range, createStream } = {} ) {
        ...
        // call transformers if not called from transformer
        if ( !this.#runningTransformer ) {
            await this.transformMessage();
        }
    ...
    createBodySync ( { range, createStream } = {} ) {
        ...
        // function
        if ( this.#isFunction ) {
            throw new Error( "Message body is function" );
        }

        // has transformers
        if ( this.hasTransformers && !this.#runningTransformer ) {
            throw new Error( "Message has transformers" );
        }
    ...
    [ Symbol.for( "nodejs.util.inspect.custom" ) ] ( depth, options, inspect ) {
        const spec = {
            ...
            "hasTransformers": this.hasTransformers,
        };
