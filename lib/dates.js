...
export const MONTHS = {
    ...
    HTTP_DATE_PATTERN = String.raw`(?<weekday>${ [ ...WEEKDAYS3 ].join( "|" ) }), (?<day>\d{2}) (?<month>${ Object.keys( MONTHS3 ).join( "|" ) }) (?<year>\d{4}) (?<hour>\d{2}):(?<minute>\d{2}):(?<second>\d{2}) GMT`,
...
export function parseInstantHttpDate ( date ) {
    const match = date.match( HTTP_DATE_REGEXP );
