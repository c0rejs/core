...
export default class Passwords {
    ...
    findCodePointAlphabet ( codePoint ) {
        const category = utf8Alphabet.findRange( codePoint )?.name;
    ...
    for ( const range of utf8Alphabet.ranges ) {
        if ( !alphabets[ range.name ] ) {
            ...
            if ( ascii.has( range.name ) ) {
                alphabets[ range.name ].tags.push( "ascii" );
            }
            else if ( range.name !== "unused" ) {
                alphabets[ range.name ].tags.push( "utf8" );
            }
        }

        alphabets[ range.name ].ranges.push( range );
    }

    for ( const alphabet of Object.values( alphabets ) ) {
        Passwords.addAlphabet( alphabet.name, alphabet.ranges, {
            "tags": alphabet.tags,
        } );
    }
}
