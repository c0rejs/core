...
export function encodeString ( value, { rfc5987 } = {} ) {
    ...
    // quote
        ...
        // encode
        value = value.replaceAll( /[\x00-\x1F\x7F]/gv, char => encodeURIComponent( char ) );
...
export function encodeEtag ( etag, { weak } = {} ) {
    ...
    if ( !etag ) {
        ...
        etag = `"${ etag.replaceAll( /[\x00-\x20"\x7F]/gv, char => encodeURIComponent( char ) ) }"`;
...
export function decodeString ( value, { utf8 = true, rfc5987, unescape = true } = {} ) {
    ...
    if ( rfc5987 ) {
        ...
        UPGRADE_TO_UTF8: if ( utf8 ) {
            ...
            // is "ascii" string
            if ( !/[^\x00-\x7F]/v.test( value ) ) break UPGRADE_TO_UTF8;
