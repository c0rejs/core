...
export function encodeValue ( value, { encode = true, quote, utf8, language } = {} ) {
    if ( value == null ) {
        return "";
    }
    else if ( typeof value === "string" ) {
        if ( utf8 ) {
            value = `utf-8'${ language || "" }'${ encodeURI( value ).replaceAll( /[,;=]/g, char => encodeURIComponent( char ) ) }`;
        }
        else {

            // encode
            if ( encode ) {
                value = encodeURI( value ).replaceAll( /[,;=]/g, char => encodeURIComponent( char ) );
            }
...
export function decodeString ( value, { unquote = true, decode = true, utf8 } = {} ) {
    if ( value == null ) {
        value = "";
    }
    else if ( typeof value !== "string" ) {
    ...
    if ( utf8 ) {
        const match = value.match( /^(?<encoding>[A-Za-z\d-_]+)'(?<language>[A-Z_a-z]*)'(?<value>.+)/ );
        if ( !match ) return;
