...
import ContentEncodingHeader from "#lib/http/headers/header/content-encoding";
...
import TeHeader from "#lib/http/headers/header/te";
import TransferEncodingHeader from "#lib/http/headers/header/transfer-encoding";
...
export default class Headers {
        ...
        if ( headers ) {
            if ( headers instanceof this.constructor ) {
                for ( let header of headers.values() ) {
                    header = new header.constructor( header );
    ...
    static registerCustomHeader ( CustomHeaderClass ) {
        ...
        Object.defineProperty( this.prototype, toCamelCase( CustomHeaderClass.headerName ), {
            "configurable": false,
            "enumerable": false,
            get () {
    ...
    deleteContentHeaders () {
        for ( const header in this.#headers ) {
            if ( header.startsWith( "content-" ) ) {
                this.#headers[ header ].delete();
            }
        }
    ...
    toString ( { crlf } = {} ) {
        const headers = Array.from( this.entries(), ( [ name, value ] ) => `${ name }: ${ value }\r\n` );
    ...
    keys () {
        return Object.keys( this.#headers ).filter( header => this.#headers[ header ].hasValue );
    }
...
Headers.registerCustomHeader( ContentEncodingHeader );
...
Headers.registerCustomHeader( TeHeader );
Headers.registerCustomHeader( TransferEncodingHeader );
