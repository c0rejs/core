...
import Request from "#lib/http/server/request";
...
const WEBSOCKET_COMPRESSIONS = new Set( [
    ...
    DEFAULT_WEBSOCKET_MAX_PAYLOAD_LENGTH = 1024 * 16, // 16 MiB
    DEFAULT_WEBSOCKET_COMPRESSION = [ "SHARED_COMPRESSOR", "SHARED_DECOMPRESSOR" ],
...
export default class HttpServer {
    ...
    constructor ( {
        ...
        if ( localTls ) {
            certificatePath ||= certificates.localCertificatePath;
    ...
    file ( location, file, { headers } = {} ) {
        location = this.#normalizeLocation( location );

        headers = new Headers( headers );

        headers.contentDisposition.set( "inline" );
    ...
    directory ( location, directory, { headers } = {} ) {
        location = this.#normalizeLocation( location );
        ...
        this.get( `${ location }/*`, req => {
            ...
            if ( filepath === "/" ) {
                message = new Message( {
                    headers,
                    "body": new File( { "path": indexPath } ),
                } );
            }
            else {
                message = new Message( {
                    headers,
                    "body": new File( { "path": path.join( directory, filepath ) } ),
                } );
            }
        ...
        this.get( `${ location }/*`, req => {
            ...
            // files from the root dir - must revalidate
                ...
                message = new Message( {
                    ...optionsRevalidate,
                    "body": new File( {
                        "path": path.join( directory, filepath ),
                    } ),
                } );
            }
            ...
            message.contentDisposition.delete();
