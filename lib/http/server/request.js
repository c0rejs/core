...
export default class Request extends Events {
    ...
    constructor ( server, req, res, socketContext ) {
        ...
        // has body
        this.#hasBody = !!( this.headers.contentLength.bytes || this.headers.get( "transfer-encoding" )?.toLowerCase().includes( "chunked" ) );
    ...
    async multipartData ( { maxBufferLength, maxFileSize } = {} ) {
        return MultipartStreamDecoder.parse( this.body, {
    ...
    async #end ( message ) {
        ...
        await message.checkHttpRange( this.headers, {
            "createBody": !methodIsHead,
            "maxRanges": 10,
        } );

        await message.checkCompression( this.headers, {
            "createBody": !methodIsHead,
            "compress": this.#server.httpCompress,
            "zlibOptions": this.#server.zlibOptions,
        } );

        const contentLength = message.contentLength;
        message.headers.delete( "content-length" );
        message.headers.delete( "transfer-encoding" );
        ...
        if ( message.hasBody ) {
            ...
            if ( methodIsHead ) {
                ...
                // know content length
                    ...
                    message.headers.set( "content-length", contentLength );
                }
            }
        }
        else {
