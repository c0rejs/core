...
const BROWSERS = await externalResources
    .add( "corejslib/core/resources/http" )
...
export default class Dispatcher extends Agent {
    ...
    dispatch ( { compress, browser, cookies, hosts, ...options }, handlers ) {
        ...
        if ( cookies ) {
            ...
            handlers.onHeaders = ( status, responseHeaders, resume, statusText ) => {
                const headers = new Headers();

                for ( let n = 0; n < responseHeaders.length; n += 2 ) {
                    if ( responseHeaders[ n ].toString( "latin1" ).toLowerCase() !== "set-cookie" ) continue;

                    headers.add( "set-cookie", responseHeaders[ n + 1 ].toString() );
                }

                cookies.add( url, headers.setCookie.cookies );

                onHeaders.call( handlers, status, responseHeaders, resume, statusText );
            };
        }
