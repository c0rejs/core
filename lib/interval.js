...
export default class Interval {
    ...
    getFormatDurationParams () {
        if ( !this.#formatDurationParams ) {
            ...
            if ( this.#formatDurationParams.years > 0xFFFF_FFFF ) {
                this.#formatDurationParams.years = 0xFFFF_FFFF;
            }
        }
    ...
    #addFractionalUnit ( value, unit ) {
        if ( UNITS[ unit ].months ) {
            ...
            if ( !value.decimal.isZero ) {
                if ( unit === "months" ) {
                    this.#units.nanoseconds = this.#units.nanoseconds.add( value.decimal.multiply( UNITS[ unit ].nanoseconds ) );
                }
                else {
    ...
    #buildUnits () {
        for ( const name in this.#units ) {
            ...
            if ( UNITS[ name ].months ) {
                if ( name !== "months" ) {
                    this.#units[ name ] = this.#units.months.divide( UNITS[ name ].months ).trunc();
            ...
            // nanoseconds
                ...
                if ( name !== "nanoseconds" ) {
                    this.#units[ name ] = this.#units.nanoseconds.divide( UNITS[ name ].nanoseconds ).trunc();
        ...
        if ( !this.#normalizedUnits ) {
            ...
            for ( const unit in this.#units ) {
                if ( UNITS[ unit ].nanoseconds > math.abs( nanoseconds ) ) {
                    ...
                    this.#normalizedUnits[ unit ] = nanoseconds / UNITS[ unit ].nanoseconds;
