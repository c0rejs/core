...
export default class Ranges {
    ...
    findRange ( value ) {
        ...
        if ( !this.#startAvlTree ) {
            ...
            for ( const range of this.#ranges ) {
                const node = this.#startAvlTree.get( range.start );

                if ( node ) {
                    node.value.ranges.push( range );
                }
                else {
                    this.#startAvlTree.set( range.start, {
                        "ranges": [ range ],
                    } );
                }
            }
        ...
        while ( node ) {
            if ( value >= node.key ) {
                for ( const range of node.value.ranges ) {
                    if ( range.hasValue( value ) ) {
                        return range;
                    }
                }
