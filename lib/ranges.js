...
import SegmentTree from "#lib/data-structures/segment-tree";
...
export default class Ranges {
    ...
    get hasIntersectingRanges () {
        if ( this.#hasIntersectingRanges == null ) {
            if ( this.size <= 1 ) {
                this.#hasIntersectingRanges = false;
            }
            ...
            // scanline algorithm
                ...
                for ( const point of points ) {
                    if ( point.start ) {
                        if ( started ) {
                            this.#hasIntersectingRanges = true;
        ...
        return this.#hasIntersectingRanges;
    }
    ...
    hasValue ( value ) {
        return !!this.findRanges( value ).length;
    }

    findRanges ( value ) {
        return this.#getSegmentTree().findRanges( value );
    }

    findIntersectingRanges ( range ) {
        return this.#getSegmentTree().findIntersectingRanges( range );
    }

    findCoveringRanges ( range ) {
        return this.#getSegmentTree().findCoveringRanges( range );
    }

    findCoveredRanges ( range ) {
        return this.#getSegmentTree().findCoveredRanges( range );
    }
    ...
    #getSegmentTree () {
        if ( !this.#segmentTree ) {
            this.#segmentTree = new SegmentTree( this.#ranges );
        }
