...
const NAME_RE = /^[\da-z\-]{1,32}$/v,
    VALUE_RE = /^[\d+.\/A-Za-z\-]+$/v,
    INTEGER_RE = /^-?(?:0|[1-9]\d{0,15})$/v,
    PHC_RE = new RegExp(
        String.raw`^` +
...
export function toPhc ( { id, version, params, salt, hash, defaultParams, sortParams } = {} ) {
    ...
    if ( params ) {
        ...
        for ( [ name, value ] of params ) {
            ...
            if ( value == null ) {
                ...
                value = value.toString( "base64" ).replace( /=+$/v, "" );
            }
    ...
    if ( salt != null ) {
        if ( typeof salt === "string" ) {
            ...
            salt = salt.toString( "base64" ).replace( /=+$/v, "" );
        }
        else {
        ...
        if ( salt.length ) {
            ...
            if ( hash != null ) {
                if ( Buffer.isBuffer( hash ) ) {
                    hash = hash.toString( "base64" ).replace( /=+$/v, "" );
                }
                else {
