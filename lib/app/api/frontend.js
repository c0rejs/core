            ...
            // post
                ...
                if ( req.headers.contentType.type === "multipart/form-data" ) {
                    ...
                    const multipartData = await req.multipartData( {
                        "maxContentLength": this.api.schema.maxUploadSize, // XXX use method maxUploadSize
                        "maxBufferLength": 1024 ** 2, // 1 MiB
                    } );

                    try {
                        const params = multipartData.fields.params.body;

                        delete multipartData.fields.params;

                        args = JSON.parse( params, ( key, value ) => {

                            // attachment
                            if ( multipartData.fields[ value ] ) {
