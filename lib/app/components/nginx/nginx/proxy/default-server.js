...
export default class NginxProxyDefaultServer {
    ...
    constructor ( nginx, { port, type, proxyProtocol, ssl } = {} ) {
        ...
        this.#proxyProtocol = Boolean( proxyProtocol );
    ...
    get listenOptions () {
        ...
        if ( this.isHttp || this.isTcp ) {
            if ( this.isDefaultServer ) {
                options.push( "default_server", this.#nginx.config.tcpListenOptions );
            }

            // XXX tcp defaultServer - always use "ssl"
            if ( this.ssl ) {
                options.push( "ssl" );
            }
        }
        else if ( this.isUdp ) {
            options.push( "udp", this.#nginx.config.udpListenOptions );
        }
    ...
    get listenOptionsQuic () {
        const options = [];

        if ( this.isHttp && this.ssl ) {
            if ( this.isDefaultServer ) {
                options.push( "default_server", this.#nginx.config.udpListenOptions );
            }
