...
export default class NginxProxyDefaultServer {
    ...
    // public
    getListenOptions ( useProxyProtocol = this.useProxyProtocol ) {
        const options = [];
    ...
    async generateConfig ( { localAddress, useProxyProtocol } = {} ) {
        var config;
        ...
        if ( this.isHttp ) {
            config = httpConfigTemplate.render( {
                ...
                "server": this,
                localAddress,
                "listenOptions": this.getListenOptions( useProxyProtocol ),
            } );
        }
        else {
