...
export default class Users {
    ...
    validatePassword ( password ) {
        const data = passwords.checkPassword( password );

        if ( !data.strong && !this.config.allowWeakPasswords ) {
    ...
    generateRandomPassword () {
        return passwords.generateRandomPassword().password;
    }
    ...
    async setUserPassword ( userId, password, { dbh, validatePassword = true } = {} ) {
        ...
        // generate password, if password is empty
        if ( !password ) {
            password = passwords.generateRandomPassword().password;
        }
