...
export default class Storage {
    ...
    async upload ( filePath, message, { cwd, expires, maxAge, inactiveMaxAge, encrypt, dbh } = {} ) {
        ...
        if ( message.isStream ) return result( [ 400, "Stream body is not supported" ] );
        ...
        const location = this.#locations.getLocation( filePath ),
            ...
            hash = await this.#getHash( message );
    ...
    async #getHash ( message ) {
        return hash( HASH_ALGORYTM, await message.createBodyStream(), {
