...
const SET_KEY = Symbol(),
    ...
    SET_LEFT = Symbol(),
    SET_RIGHT = Symbol(),
    SET_BALANCE_FACTOR = Symbol(),
    ADD_BALANCE_FACTOR = Symbol();
...
export class AvlTreeNode {
    ...
    [ SET_BALANCE_FACTOR ] ( value ) {
        this.#balanceFactor = value;

        return this;
    }

    [ ADD_BALANCE_FACTOR ] ( value ) {
        this.#balanceFactor += value;
...
export default class AvlTree {
    ...
    constructor ( { compare } = {} ) {
        this.#compare = compare || utilsCompare;
    }
    ...
    set ( key, value ) {
        ...
        if ( typeof key === "object" ) {
            ...
            if ( this.size ) {
                ...
                this.#sort( entries, 0, size - 1, this.#compare );
    ...
    * [ Symbol.iterator ] () {
    ...
    #load ( parent, entries, start, end ) {
        ...
        if ( size > 0 ) {
            ...
            node[ SET_LEFT ]( this.#load( node, entries, start, middle ) );
            node[ SET_RIGHT ]( this.#load( node, entries, middle + 1, end ) );
    ...
    #rotateLeft ( node ) {
        ...
        node[ SET_RIGHT ]( rightNode.left );

        if ( rightNode.left ) rightNode.left[ SET_PARENT ]( node );

        rightNode[ SET_PARENT ]( node.parent );
        if ( rightNode.parent ) {
            if ( rightNode.parent.left === node ) {
                rightNode.parent[ SET_LEFT ]( rightNode );
            }
            else {
                rightNode.parent[ SET_RIGHT ]( rightNode );
            }
        }

        node[ SET_PARENT ]( rightNode );
        rightNode[ SET_LEFT ]( node );

        node[ ADD_BALANCE_FACTOR ]( 1 );
        if ( rightNode.balanceFactor < 0 ) {
            node[ ADD_BALANCE_FACTOR ]( -rightNode.balanceFactor );
        }

        rightNode[ ADD_BALANCE_FACTOR ]( 1 );
        if ( node.balanceFactor > 0 ) {
            rightNode[ ADD_BALANCE_FACTOR ]( node.balanceFactor );
        }
    ...
    #rotateRight ( node ) {
        ...
        node[ SET_LEFT ]( leftNode.right );
        if ( node.left ) node.left[ SET_PARENT ]( node );

        leftNode[ SET_PARENT ]( node.parent );
        if ( leftNode.parent ) {
            if ( leftNode.parent.left === node ) {
                leftNode.parent[ SET_LEFT ]( leftNode );
            }
            else {
                leftNode.parent[ SET_RIGHT ]( leftNode );
            }
        }

        node[ SET_PARENT ]( leftNode );
        leftNode[ SET_RIGHT ]( node );

        node[ ADD_BALANCE_FACTOR ]( -1 );
        if ( leftNode.balanceFactor > 0 ) {
            node[ ADD_BALANCE_FACTOR ]( -leftNode.balanceFactor );
        }

        leftNode[ ADD_BALANCE_FACTOR ]( -1 );
        if ( node.balanceFactor < 0 ) {
            leftNode[ ADD_BALANCE_FACTOR ]( node.balanceFactor );
        }
