...
const INTEGER_RE = /^-?\d+$/v;

// public
export default function expandBraces ( pattern ) {
    const sequences = {},
        ast = scanContent( pattern, 0, false ).nodes,
        patterns = cartesianProduct( ast.map( node => expandNode( node, sequences, {} ) ) );
...
function parseSequence ( value ) {
    ...
    const startType = INTEGER_RE.test( start )
            ? 1
            : start.length === 1
                ? 2
                : 3,
        endType = INTEGER_RE.test( end )
            ? 1
            : end.length === 1
                ? 2
                : 4;
    ...
    if ( parts.length === 3 ) {
        ...
        if ( !INTEGER_RE.test( step ) ) return;
