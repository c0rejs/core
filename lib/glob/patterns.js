...
export default class GlobPatterns {
    ...
    test ( testPath, { prefix, absolute, normalize, listMode } = {} ) {
        let allow = false;
        ...
        if ( prefix || absolute || normalize ) {
            if ( isArray ) {
                testPath = testPath.map( path => GlobPattern.normalizePath( path, { prefix, absolute } ) );
            }
            else {
        ...
        if ( listMode ) {
            ...
            // alloewed and has no denied patterns
