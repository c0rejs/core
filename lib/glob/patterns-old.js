...
export class GlobPattern {
    ...
    constructor ( pattern, { caseSensitive = true, prefix, relativeGlobstar } = {} ) {
        ...
        // unescape
        pattern = pattern.replaceAll( /\\(.)/gv, ( match, escape ) => {
        ...
        // normalize
        this.#unescapedPattern = path.posix.join( "/", prefix || "", pattern, "." ).replaceAll( /\/\*\*(?:\/\*\*)+/gv, "/**" );
    }

    // static
    static new ( pattern, { caseSensitive = true, prefix, relativeGlobstar } = {} ) {
        if ( pattern instanceof this ) return pattern;
