...
const MAX_PRECISION = 1000,
    ...
    DEFAULT_SCALE = 16,
    DEFAULT_ROUND_MODE = "round",
    DEFAULT_MODULO_MODE = "trunc",
    ROUND_MODES = new Set( [ "round", "trunc", "floor", "ceil" ] ),
...
class Numeric {
    ...
    constructor ( value, { precision, scale, roundMode, moduloMode, valueScale } = {} ) {
        if ( precision ) {
        ...
        this.#ROUND_MODE = roundMode || DEFAULT_ROUND_MODE;
        ...
        if ( !Number.isSafeInteger( this.#PRECISION ) || this.#PRECISION <= 0 || this.#PRECISION > MAX_PRECISION ) {
            throw new Error( "Numeric pprecision must be a positive integer" );
        }
        ...
        if ( !ROUND_MODES.has( this.#ROUND_MODE ) ) {
            throw new Error( `Numeric roundMode "${ this.#ROUND_MODE }" is not valid` );
        }
        ...
        if ( typeof value === "bigint" ) {
            ...
            // calculate precision
            this.#rawPrecision ??= getBigIntPrecision( value );
    ...
    get precision () {
        if ( this.#precision === undefined ) {
            if ( this.isFinite ) {
                this.#precision = getBigIntPrecision( this.value );
            }
            else {
                this.#precision = 0;
            }
    ...
    get isInteger () {
        return this.isFinite && this.#rawScale === 0;
    }

    get isSafeInteger () {
        return this.isInteger && this.#rawValue <= this.constructor.MAX_SAFE_INTEGER.rawValue && this.#rawValue >= this.constructor.MIN_SAFE_INTEGER.rawValue;
    }
    ...
    toNumeric ( { precision, scale, roundMode, moduloMode } = {} ) {
        return new this.constructor( this.#rawValue, {
            ...
            "scale": scale === undefined
                ? this.#SCALE
                : scale,
            "roundMode": roundMode || this.#ROUND_MODE,
    ...
    round ( scale ) {
        if ( !this.isFinite ) {
            ...
            if ( scale >= this.#rawScale ) {
                return this;
            }
            else {
                return this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, scale, "round" ), scale );
            }
        }
        else {
            if ( !this.#rounded ) {
                this.#rounded = this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, 0, "round" ), 0 );
            }
    ...
    trunc ( scale ) {
        if ( !this.isFinite ) {
            ...
            if ( scale >= this.#rawScale ) {
                ...
                return this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, scale, "trunc" ), scale );
            }
        }
        else {
            if ( !this.#truncated ) {
                this.#truncated = this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, 0, "trunc" ), 0 );
            }
    ...
    floor ( scale ) {
        if ( !this.isFinite ) {
            ...
            if ( scale >= this.#rawScale ) {
                ...
                return this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, scale, "floor" ), scale );
            }
        }
        else {
            if ( !this.#floored ) {
                this.#floored = this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, 0, "floor" ), 0 );
            }
    ...
    ceil ( scale ) {
        if ( !this.isFinite ) {
            ...
            if ( scale >= this.#rawScale ) {
                ...
                return this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, scale, "ceil" ), scale );
            }
        }
        else {
            if ( !this.#ceiled ) {
                this.#ceiled = this.#createNumeric( roundBigInt( this.#rawValue, this.#rawScale, 0, "ceil" ), 0 );
            }
    ...
    add ( numeric ) {
        ...
        if ( this.isFinite && numeric.isFinite ) {
            const commonScale = Math.max( this.#rawScale, numeric.rawScale ),
                a = this.#rawValue * pow10( commonScale - this.#rawScale ),
                b = numeric.rawValue * pow10( commonScale - numeric.rawScale );

            return this.#createNumeric( ...this.#capScale( a + b, commonScale ) );
        }
        else {
    ...
    multiply ( numeric ) {
        ...
        if ( this.isFinite && numeric.isFinite ) {
            const productScale = this.#rawScale + numeric.rawScale,
                product = this.#rawValue * numeric.rawValue;

            return this.#createNumeric( ...this.#capScale( product, productScale ) );
        }
        else {
    ...
    divide ( numeric ) {
        ...
        if ( this.isFinite && numeric.isFinite ) {
            if ( numeric.rawValue === 0n ) {
                ...
                let scale = Math.max( rscale, this.#SCALE ) + 1;
                ...
                // #ROUND_MODE contract used everywhere else in the class
                if ( remainder !== 0n ) {
                    ...
                    switch ( this.#ROUND_MODE ) {
                        ...
                        case "round":
                        default:
                            roundUp = absRemainder * 2n >= absDenominator;
                            break;
                    }
        ...
        if ( this.isFinite && numeric.isFinite ) {
            if ( numeric.rawValue === 0n ) {
                ...
                if ( remainder === 0n ) {
                    ...
                    // remove trailing zeros in fractional part
                    while ( scale > 0 && remainder % 10n === 0n ) {
                        remainder /= 10n;
                        scale--;
                    }
                }

                return this.#createNumeric( remainder, scale );
            }
        }
        else {
    ...
    sqrt () {
        if ( !this.#sqrt ) {
            if ( this.isNaN || this.isNegativeInfinity || ( this.isFinite && this.#rawValue < 0n ) ) {
                ...
                let quotient = isqrtBigInt( radicand );
    ...
    #materialize () {
        if ( !this.isFinite ) {
            ...
            if ( this.#rawScale <= targetScale ) {
                ...
                const roundedValue = roundBigInt( this.#rawValue, this.#rawScale, targetScale, this.#ROUND_MODE );
                ...
                const roundedDigits = ( roundedValue < 0n
                        ? -roundedValue
                        : roundedValue ).toString().length,
    ...
    #capScale ( value, scale ) {
        ...
        return [ roundBigInt( value, scale, this.#PRECISION, this.#ROUND_MODE ), this.#PRECISION ];
    }
...
function parseString ( value ) {
    ...
    if ( /^[+\-]?0[bxo]/iv.test( value ) ) {
        ...
        if ( !match ) {
            ...
            let scale = fractionalDigits.length * radixInfo.bits,
                unsignedValue = integerValue * pow10( scale );
            ...
            // remove trailing zeros in fractional part
            while ( scale > 0 && unsignedValue % 10n === 0n ) {
                unsignedValue /= 10n;
                scale--;
            }
...
function roundBigInt ( value, fromScale, toScale, mode ) {
    ...
    switch ( mode ) {
        ...
        case "round": {
            ...
            roundUp = absRemainder * 2n >= divisor;
            break;
        }
        default:
            roundUp = false;
...
function getBigIntPrecision ( value ) {
    return ( value < 0n
        ? -value
