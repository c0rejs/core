...
export default class Blob extends globalThis.Blob {
    ...
    stream ( { range, type } = {} ) {
        ...
        if ( !range || range.isFullRange ) {
            readableStream = stream.Readable.fromWeb( super.stream() );

            readableStream.setContentLength( this.size );
        }
        else {
            readableStream = this.#slice( range ).stream();
        }

        return readableStream.setContentType( type === undefined
            ? this.type
            : type );
    }
    ...
    async toDataUrl ( { encoding = "base64" } = {} ) {
    ...
    toJSON () {
        throw new Error( "Unable to serialize Blob to JSON" );
    }
