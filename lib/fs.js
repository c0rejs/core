...
export function sanitizeFilename ( filename, { windows = true } = {} ) {
    const replacement = "_";

    filename = filename.replaceAll( illegalRe, replacement ).replaceAll( controlRe, replacement ).replace( reservedRe, replacement );

    if ( windows ) {
        filename = filename.replace( windowsReservedRe, `$1${ replacement }$2` ).replace( windowsTrailingRe, replacement );
    }
