...
export default async function fetch ( url, { compress, browser, cookies, headersTimeout, bodyTimeout, reset, blocking, ...options } = {} ) {
    ...
    await using message = new Message( {
    ...
    if ( message.hasBody ) {
        ...
        options.body = await message.createBody();
    }
    ...
    options.dispatcher = {
        dispatch ( options, handlers ) {
            ...
            const onResponseStart = handlers.onResponseStart;

            handlers.onResponseStart = ( controller, status, responseHeaders, statusText ) => {
                responseStatus = [ status, statusText ];

                onResponseStart.call( handlers, controller, status, responseHeaders, statusText );
            };
    ...
    try {
        ...
        if ( responseStatus?.[ 0 ] === 407 ) {
            ...
            res = result.fromError( e.cause, { "log": false } );
        }
    }
