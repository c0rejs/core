...
export default class IpRanges extends Events {
    ...
    includes ( range ) {
        ...
        if ( range.isIpV4 ) {
            return this.#searchAvl( range, this.#avlIpV4 );
        }
        else {
            return this.#searchAvl( range, this.#avlIpV6 );
        }
    }
    ...
    #searchAvl ( range, avl ) {
        ...
        return this.#searchAvlNode( range, avl.root );
    }

    #searchAvlNode ( range, node ) {

        // range >= node
        if ( range.firstAddress.value >= node.value.firstAddress.value ) {

            // range found
            if ( range.lastAddress.value <= node.value.lastAddress.value ) {
                return node.value;
            }

            if ( node.left ) {
                const foundRange = this.#searchAvlNode( range, node.left );

                if ( foundRange ) return foundRange;
            }

            if ( node.right ) {
                const foundRange = this.#searchAvlNode( range, node.right );
        ...
        // range < node
            ...
            if ( node.left ) {
                return this.#searchAvlNode( range, node.left );
            }
        }
    }
