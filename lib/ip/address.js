...
try {
    var { "default": geoipCity } = await import( "@corejslib/geoip-city" );
}
catch {}

try {
    var { "default": geoipAsn } = await import( "@corejslib/geoip-asn" );
}
catch {}

const PARSING_ERROR_MESSAGE = "IP address is invalid",
    MAX_IPV4_VALUE = 4_294_967_295,
    MAX_IPV6_VALUE = 340_282_366_920_938_463_463_374_607_431_768_211_455n,
    MAX_IPV4_VALUE_LENGTH = MAX_IPV4_VALUE.toString( 16 ).length,
    MAX_IPV6_VALUE_LENGTH = MAX_IPV6_VALUE.toString( 16 ).length;
...
export default class IpAddress {
    ...
    get id () {
        if ( !this.#id ) {
            if ( this.isIpV4 ) {
                this.#id = this.value.toString( 16 ).padStart( MAX_IPV4_VALUE_LENGTH, "0" );
            }
            else {
                this.#id = this.value.toString( 16 ).padStart( MAX_IPV6_VALUE_LENGTH, "0" );
            }
        }
    ...
    toString () {
        if ( this.#string === undefined ) {
            if ( this.#isIpV4 ) {
                ...
                this.#string = Array.from( buf ).join( "." );
    ...
    toFullString () {
        if ( this.#fullString === undefined ) this.toString();
    ...
    isCoveredBy ( ipRange ) {
        ...
        return ipRange.contains( this );
    }

    compare ( ipAddress ) {
        ipAddress = this.constructor.new( ipAddress );

        return compare( this.family, ipAddress.family ) || compare( this.#value, ipAddress.value );
    }
    ...
    #parseV4 ( address ) {
        ...
        if ( octets.length !== 4 ) throw new Error( PARSING_ERROR_MESSAGE );

        for ( let n = 0; n < octets.length; n++ ) {

            // strictly decimal digits only, no empty strings, signs, hex or exponent notation
            if ( !/^\d{1,3}$/v.test( octets[ n ] ) ) throw new Error( PARSING_ERROR_MESSAGE );

            const number = Number( octets[ n ] );

            if ( number > 255 ) throw new Error( PARSING_ERROR_MESSAGE );

            octets[ n ] = number;
        }

        this.#isIpV4 = true;
        this.#value = octets[ 0 ] * 16_777_216 + octets[ 1 ] * 65_536 + octets[ 2 ] * 256 + octets[ 3 ];
    }
