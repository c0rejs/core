...
export default class DigitalSize {
            ...
            if ( typeof size === "string" ) {
                ...
                if ( !Number.isNaN( number ) ) {
                    ...
                    if ( bytes != null ) {
                        ...
                        const match = size.split( /([+\-]?\d+(?:\.\d+)?)([A-Za-z]+)/v );
    ...
    toNginx ( { offset } = {} ) {
        ...
        if ( this.#string[ id ] == null ) {
            if ( this.#bytes ) {
                ...
                // default
                this.#string[ id ] ??= String( this.#bytes );
            }
            else {
                this.#string[ id ] = "";
            }
    ...
    getFormatDigitalSizeParam () {
        if ( !this.#formatDigitalSizeParam ) {
            const bytes = Math.abs( this.#bytes ),
                unitsBytes = UNIT_KILO_BYTES;

            for ( const unit in unitsBytes ) {
                if ( bytes >= unitsBytes[ unit ] ) {
                    this.#formatDigitalSizeParam = {
                        unit,
        ...
        return this.#formatDigitalSizeParam;
    }
