...
export default class File extends Blob {
        ...
        if ( typeof filePath === "object" ) {
            ...
            if ( filePath instanceof URL ) {
                ...
                if ( filePath.protocol === "data:" ) {
                    ...
                    filePath = dataUrl.searchParams.get( "filename" );
                }
    ...
    stream ( { name, type, range } = {} ) {
        ...
        if ( this.#hasSources ) {
            return super.stream( { range, type } ).setFilename( name === undefined
                ? this.name
                : name );
        }
        else if ( range ) {
            ...
            if ( range.length === 0 ) {
                ...
                readableStream = stream.Readable.from( "", { "objectMode": false } );
            }
            else {
            ...
            readableStream.setContentLength( range.length );
        }
        else {
            readableStream = fs.createReadStream( this.#path ).setContentLength( this.size );
        }

        return readableStream.setFilename( name === undefined
            ? this.name
            : name ).setContentType( type === undefined
            ? this.type
            : type );
    }
    ...
    async toDataUrl ( { encoding = "base64", withFilename = true } = {} ) {
        const url = new DataUrl();

        if ( withFilename && this.name ) url.searchParams.set( "filename", this.name );
