...
export default class File extends Blob {
    ...
    stream ( { name, type, lastModified, range } = {} ) {
        var readableStream;

        if ( this.#hasSources ) {
            readableStream = super.stream( { range } );
        }
        else if ( range ) {
            ...
            readableStream.setContentLength( this.size );
        }

        return readableStream //
            .setContentType( type === undefined
                ? this.type
                : type )
            .setFilename( name === undefined
                ? this.name
                : name )
            .setLastModified( lastModified === undefined
                ? this.lastModified
                : lastModified );
    }
